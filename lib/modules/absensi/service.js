...
export async function handleAbsensiRequest(req, subPath, method, ctx) {
  ...
  if ((subPath === 'report' || subPath === 'report/export') && method === 'GET') {
    ...
    // Kolom SO + Lembur ditambahkan (additive) — kolom existing tetap.
    const header = [
      ...
      // Lembur
      'Potensi Lembur (menit)',
      'Status Lembur', 'Jam Mulai Lembur', 'Jam Selesai Lembur',
      'Durasi Lembur (menit)', 'Alasan Lembur',
      'Approver', 'Ditinjau At', 'Foto Lembur',
      'Foto Selfie',
      // Verifikasi (GPS + Foto)
      'Status Foto Masuk', 'Latitude Masuk', 'Longitude Masuk',
      'Jarak Masuk (m)', 'Radius Masuk (m)', 'Status GPS Masuk',
      'Status Foto Keluar', 'Latitude Keluar', 'Longitude Keluar',
      'Jarak Keluar (m)', 'Radius Keluar (m)', 'Status GPS Keluar',
    ];
    const aoa = [header];
    for (const r of rows) {
      // Derive SO fields
      const soOn = !!r.so_selected;
      const soStart = soOn ? (r.actual_check_in_wita || '') : '';
      const soEffStart = soOn && r.so_effective_start_mins != null
        ? `${String(Math.floor(r.so_effective_start_mins / 60)).padStart(2, '0')}:${String(r.so_effective_start_mins % 60).padStart(2, '0')}`
        : '';
      // Lembur fields — jam mulai = shift_end, selesai = actual_check_out.
      // Durasi diakui = overtime_minutes (0 jika ditolak / tidak diajukan).
      const otStatus = r.overtime_status || 'none';
      const otRecognized = otStatus === 'approved' ? Number(r.overtime_minutes || 0) : 0;
      const otEndWita = r.actual_check_out_wita || '';
      const otStartWita = (r.overtime_requested && r.shift_end) ? r.shift_end : '';
      const otPotensi = Number(r.overtime_raw_minutes ?? r.overtime_minutes ?? 0);
      // Verifikasi fields
      const photoInStatus = r.selfie_deleted ? 'Dihapus (retensi)' : (r.has_check_in_photo ? 'Ada' : 'Tidak ada');
      const photoOutStatus = r.selfie_deleted ? 'Dihapus (retensi)' : (r.has_check_out_photo ? 'Ada' : 'Tidak ada');
      const gpsInStatus = r.check_in_distance_m == null
        ? 'N/A' : (Number(r.check_in_distance_m) <= radiusM ? 'Valid' : 'Tidak Valid');
      const gpsOutStatus = r.check_out_distance_m == null
        ? 'N/A' : (Number(r.check_out_distance_m) <= radiusM ? 'Valid' : 'Tidak Valid');
      aoa.push([
        r.date || '',
        r.user_name || '',
        r.user_role || '',
        ...
        r.selfie_deleted ? 'Dihapus (retensi)' : (r.has_overtime_photo ? 'Ada' : 'Tidak ada'),
    ...
    ws['!cols'] = [
      ...
      { wch: 18 },
      // Verifikasi Masuk
      { wch: 14 }, { wch: 12 }, { wch: 12 }, { wch: 10 }, { wch: 10 }, { wch: 14 },
      // Verifikasi Keluar
      { wch: 14 }, { wch: 12 }, { wch: 12 }, { wch: 10 }, { wch: 10 }, { wch: 14 },
    ];
