...
export async function handleAbsensiRequest(req, subPath, method, ctx) {
  ...
  if ((subPath === 'report' || subPath === 'report/export') && method === 'GET') {
    ...
    // Agregat pakai data existing:
    //   Jam Kerja Diakui = SUM(worked_minutes) minus SUM(overtime_minutes bila approved dan
    //     worked_minutes memasukkan overtime)  ==> KEEP SIMPLE: pakai definisi
    //     "worked_minutes" apa adanya karena backend menghitungnya sebagai
    //     nowMins - effective_check_in_mins (sudah termasuk lembur yang bekerja).
    //     Untuk sheet Rekap kita pisahkan: Jam Kerja Diakui = worked_minutes − overtime_minutes(approved).
    //   Jam SO Diakui   = SUM(worked_minutes untuk rec.so_selected) mengikuti effective SO.
    //     (subset dari Jam Kerja Diakui — dijadikan "dari mana" sub-totalnya).
    //   Jam Lembur Diakui = SUM(overtime_minutes untuk overtime_status === 'approved').
    //   Total Jam Diakui = Jam Kerja Diakui + Jam Lembur Diakui.
    //   (Jam SO Diakui ditampilkan sebagai info; tidak double-count.)
    const rekapMap = new Map(); // user_id => { name, work, so, ot, total }
    for (const r of rows) {
      const uid = r.user_id;
      if (!uid) continue;
      ...
      const approvedOT = r.overtime_status === 'approved' ? Number(r.overtime_minutes || 0) : 0;
      // Jam kerja "bersih" = jam yang efektif tapi bukan porsi lembur approved.
      // worked_minutes backend = nowMins - effective_check_in_mins (mencakup lembur
      // saat staff bekerja melewati shift_end); pisahkan supaya rekap tidak
      // double-count.
      const workOnly = Math.max(0, worked - approvedOT);
      bucket.workMins += workOnly;
      bucket.overtimeMins += approvedOT;
      if (r.so_selected) bucket.soMins += workOnly;
    ...
    const rekapRows = Array.from(rekapMap.values())
      ...
      .map((b) => {
        ...
        return [b.name, work, so, ot, Math.round((work + ot) * 100) / 100];
      });
    const rekapAOA = [
    ...
    // Aktual = check_out - check_in aktual.  Kalau salah satu kosong → '-'.
    // Diakui = worked_minutes existing (source of truth).
    const jamKerjaAOA = [
      ['Tanggal', 'Nama Staff', 'Jam Kerja Normal (jam)', 'Jam Kerja Aktual (jam)', 'Jam Kerja Diakui (jam)'],
      ...rows.map((r) => {
        const normalMins = (Number(r.shift_end_mins) || 0) - (Number(r.shift_start_mins) || 0);
        let aktualMins = null;
        if (r.actual_check_in && r.actual_check_out) {
          const inDate = new Date(r.actual_check_in);
          const outDate = new Date(r.actual_check_out);
          aktualMins = Math.max(0, Math.round((outDate.getTime() - inDate.getTime()) / 60000));
        }
        return [
          r.date || '',
          r.user_name || '',
          fmtMinsToHours(normalMins),
          aktualMins == null ? '-' : fmtMinsToHours(aktualMins),
          fmtMinsToHours(r.worked_minutes || 0),
        ];
      }),
    ];

    // ---- Sheet 5: Stock Opname --------------------------------------------
    // Hanya baris yang so_selected=true. Semua field reuse existing.
    const soAOA = [
      ...
      ...rows.filter((r) => r.so_selected).map((r) => {
        ...
        // Jam Kerja Efektif SO ditampilkan sebagai rentang start–shift_end (atau
        // sampai actual_check_out kalau sudah keluar sebelum shift_end).
    ...
    // ---- Sheet 6: Lembur ---------------------------------------------------
    // Baris untuk semua yang overtime_requested=true (pengajuan). Approve→diakui;
    // Reject→0. Potensi selalu overtime_raw_minutes.
    const lemburAOA = [
      ...
      ...rows.filter((r) => r.overtime_requested || r.overtime_minutes > 0).map((r) => {
        const status = r.overtime_status || 'none';
        const diakui = status === 'approved' ? Number(r.overtime_minutes || 0) : 0;
