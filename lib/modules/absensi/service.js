...
export async function handleAbsensiRequest(req, subPath, method, ctx) {
  ...
  if ((subPath === 'report' || subPath === 'report/export') && method === 'GET') {
    ...
    // Real-time poin per staff — reuse computeLeaderboard existing.
    // Selalu pakai period saat ini (bukan filter periode laporan) supaya
    // "Poin" di laporan = angka yang sama persis dengan Live Board.
    let pointsByUser = {};
    const currentPeriodKey = periodKeyForDate(witaDate());
    try {
      const board = await computeLeaderboard(db, currentPeriodKey);
      for (const it of (board?.items || [])) {
        pointsByUser[it.user_id] = { balance: it.balance, rank: it.rank, capped: !!it.capped };
      }
    ...
    if (subPath === 'report') {
      return jsonRes({
        ...
        points_period: currentPeriodKey,
      });
    }
    ...
    const rekapRows = Array.from(rekapMap.entries())
      ...
      .map((b) => {
        ...
        // Poin real-time dari Live Board (period saat ini). Kolom ini
        // mengikuti nilai yang sama persis dilihat staff di leaderboard.
        const p = pointsByUser[b.uid];
        const poin = p ? Number(p.balance || 0) : '-';
        return [b.name, work, so, ot, total, poin];
      });
    const rekapAOA = [
      ['Nama Staff', 'Total Jam Kerja Diakui (jam)', 'Total Jam SO Diakui (jam)', 'Total Jam Lembur Diakui (jam)', 'Total Jam Diakui (jam)', 'Poin Saat Ini'],
      ...rekapRows,
    ];
