...
async function backfillCheckinLedger(db, { from, to }) {
  ...
  for (const r of records) {
    ...
    const label =
      lateM <= 0 ? 'Tepat waktu' :
      lateM < 10 ? 'Terlambat <10 menit' :
      lateM <= 30 ? 'Terlambat 10–30 menit' :
      'Terlambat >30 menit';
    const doc = {
      id: uuidv4(),
      user_id: r.user_id,
  ...
  if (subPath === 'check-in' && method === 'POST') {
    ...
    try {
      ...
      const ledger = {
        ...
        reason: (doc.late_minutes <= 0
          ? 'Tepat waktu'
          : doc.late_minutes < 10
          ? 'Terlambat <10 menit'
          : doc.late_minutes <= 30
          ? 'Terlambat 10–30 menit'
          : 'Terlambat >30 menit') + ` (${doc.shift_name || doc.shift_key})`,
        source_id: doc.id,
