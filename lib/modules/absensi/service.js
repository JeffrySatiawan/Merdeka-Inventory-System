...
export async function handleAbsensiRequest(req, subPath, method, ctx) {
  ...
  if (subPath === 'settings' && method === 'PUT') {
    ...
    if (Array.isArray(body.shifts)) {
      // Terima format jam "HH:MM" ATAU "HH.MM" (beberapa browser id-ID
      // mengembalikan value time input dengan pemisah titik). Normalisasi
      // ke "HH:MM" sebelum simpan supaya konsumen existing (parser lokasi
      // & derivasi Jam Kerja Diakui) tidak perlu ikut diubah.
      const normHM = (v) => {
      ...
      const shifts = body.shifts
        ...
        .filter((sh) => sh.key && sh.name && sh.start && sh.end);
      // Dedupe keys
      const seen = new Set();
  ...
  // -------- REKAP (visualisasi kalender) --------
  // GET /api/absensi/rekap?from=YYYY-MM-DD&to=YYYY-MM-DD
  // ADDITIVE + safe-for-staff: hanya field ringkas kehadiran (tanpa foto,
  // koordinat GPS, atau data poin). Semua user login boleh akses (owner &
  // staff) — endpoint `/report` TETAP owner-only. Reuse `absensi_records`
  // apa adanya; tidak ada database/service baru.
  if (subPath === 'rekap' && method === 'GET') {
    const url = new URL(req.url);
    const from = url.searchParams.get('from');
    const to = url.searchParams.get('to');
    const filter = {};
    ...
    const rows = await db.collection('absensi_records')
      ...
      .limit(5000)
      .toArray();
    // Daftar staff aktif (role != owner) supaya frontend bisa menampilkan
    // seluruh baris karyawan walaupun belum absen di rentang itu.
    const staff = await db.collection('employees')
      .find({ role: { $ne: 'owner' }, status: { $ne: 'inactive' } })
      .project({ _id: 0, id: 1, name: 1 })
      .sort({ name: 1 })
      .toArray();
    return jsonRes({ items: rows, staff });
  }
  ...
  if ((subPath === 'report' || subPath === 'report/export') && method === 'GET') {
    ...
    // Poin Saat Absen — snapshot poin per record berdasarkan Riwayat Poin
    // existing (`absensi_point_ledger`). Nilai dibekukan pada waktu absensi:
    // perubahan poin setelah tanggal record tidak boleh mengubah angka pada
    // laporan historis. Reuse ledger + point settings existing; tidak ada
    // collection/service baru.
    let pointsByRecord = {}; // record_id => { balance, capped }
    let pointsByUser = {};   // user_id => balance snapshot pada record TERAKHIR di filter (untuk Rekap)
    try {
      const ps = await loadPointSettings(db);
      const initialBal = Number(ps.initial_balance ?? 100);
      const maxPos = Number(ps.max_positive ?? 150);
      const maxNeg = Number(ps.max_negative ?? -50);
      const clampBal = (n) => Math.max(maxNeg, Math.min(maxPos, n));

      const involvedUserIds = Array.from(new Set(rows.map((r) => r.user_id).filter(Boolean)));
      const maxRecordDate = rows.reduce((m, r) => (r.date && (!m || r.date > m)) ? r.date : m, null);

      if (involvedUserIds.length > 0 && maxRecordDate) {
        // Ambil semua entri ledger utk user2 tsb sampai tanggal record terakhir.
        // Include entri sebelum periode filter supaya "balance at date" akurat
        // (initial + kumulatif delta hingga tanggal record, di-clamp).
        const ledger = await db.collection('absensi_point_ledger')
          .find({ user_id: { $in: involvedUserIds }, event_date: { $lte: maxRecordDate } })
          .project({ user_id: 1, event_date: 1, points: 1 })
          .sort({ event_date: 1, createdAt: 1 })
          .toArray();

        // Bangun deret kumulatif per user: [{date, cum}, ...] terurut oleh date.
        const cumByUser = new Map();
        const runByUser = new Map();
        for (const l of ledger) {
          const cur = runByUser.get(l.user_id) || 0;
          const next = cur + Number(l.points || 0);
          runByUser.set(l.user_id, next);
          const arr = cumByUser.get(l.user_id) || [];
          arr.push({ date: l.event_date, cum: next });
          cumByUser.set(l.user_id, arr);
        }
        const findCumAsOf = (uid, date) => {
          const arr = cumByUser.get(uid);
          if (!arr || arr.length === 0) return 0;
          let lo = 0, hi = arr.length - 1, ans = -1;
          while (lo <= hi) {
            const m = (lo + hi) >> 1;
            if (arr[m].date <= date) { ans = m; lo = m + 1; } else hi = m - 1;
          }
          return ans >= 0 ? arr[ans].cum : 0;
        };

        // Snapshot per record.
        const lastByUser = new Map();
        for (const r of rows) {
          if (!r.id || !r.user_id || !r.date) continue;
          const delta = findCumAsOf(r.user_id, r.date);
          const uncapped = initialBal + delta;
          const balance = clampBal(uncapped);
          pointsByRecord[r.id] = { balance, capped: balance !== uncapped };
          const cur = lastByUser.get(r.user_id);
          if (!cur || r.date > cur.date) lastByUser.set(r.user_id, { date: r.date, balance, capped: balance !== uncapped });
        }
        for (const [uid, v] of lastByUser.entries()) {
          pointsByUser[uid] = { balance: v.balance, capped: !!v.capped };
        }
      }
    } catch { /* points snapshot optional */ }
    ...
    if (subPath === 'report') {
      return jsonRes({
        ...
        // Snapshot poin pada saat absensi (frozen historic). Dikirim per-record
        // supaya tabel Laporan menampilkan angka yang benar utk baris tsb.
        points_by_record: pointsByRecord,
        // BWC: nilai rekap per staff (poin pada record terakhir dalam filter).
    ...
    const rekapRows = Array.from(rekapMap.entries())
      ...
      .map((b) => {
        ...
        const total = Math.round((work + so + ot) * 100) / 100;
        // Poin Saat Absen — snapshot poin pada record TERAKHIR staff di dalam
        // filter (frozen historic). Diambil dari ledger via `pointsByUser`
        // di atas — bukan saldo terbaru. Perubahan poin setelah tanggal itu
        // tidak mengubah angka pada laporan historis.
        const p = pointsByUser[b.uid];
        const poin = p ? Number(p.balance || 0) : '-';
        return [b.name, work, so, ot, total, poin];
      });
    const rekapAOA = [
      ['Nama Staff', 'Total Jam Kerja Diakui (jam)', 'Total Jam SO Diakui (jam)', 'Total Jam Lembur Diakui (jam)', 'Total Jam Diakui (jam)', 'Poin Saat Absen'],
      ...rekapRows,
    ];
    ...
    XLSX.utils.book_append_sheet(wb, withCols(rekapAOA, [22, 22, 20, 22, 20, 14]), 'Rekapitulasi');
