...
async function ensureIndexes(db) {
  try {
    ...
    await db.collection('mis_faktur').createIndex({ no_ketoko: 1 });
...
export async function handleFakturRequest(req, subPath, method, ctx) {
  ...
  if (subPath === '' && method === 'GET') {
    ...
    if (qStr) {
      filter.$or = [
        { no_ketoko: { $regex: qStr, $options: 'i' } },
  ...
  if (subPath === '' && method === 'POST') {
    ...
    const noKetoko = String(form.get('no_ketoko') || '').trim();
    if (!noKetoko) return errRes('No. Transaksi KETOKO wajib diisi');
  ...
  if (!action && method === 'PATCH') {
    ...
    const upd = {};
    if (body.no_ketoko !== undefined) {
      const v = String(body.no_ketoko).trim();
      if (!v) return errRes('No. Transaksi KETOKO tidak boleh kosong');
      upd.no_ketoko = v;
    }
  ...
  if (action === 'retry' && method === 'POST') {
    ...
    const caption = [
      row.no_ketoko ? `KETOKO: ${row.no_ketoko}` : null,
