...
async function handlePDFRequest(req, subPath, method, { db, user }) {
  ...
  if (subPath === 'pdfs' && method === 'GET') {
    ...
    const items = await db
      ...
      .toArray();
    // Include server timestamp so client can use it as cursor for next poll.
    return jsonRes({ items, server_time: new Date().toISOString() });
  }

  // GET /api/om/pdfs/[id]/file — stream PDF binary (inline)
