...
export async function handleOMRequest(req, subPath, method, { db, user }) {
  ...
  if (subPath === 'scan/print' && method === 'POST') {
    ...
    if (existing) {
      // If already printed by same session (or anyone), block duplicate to keep counters accurate
      if (existing.printed_at) {
        return err(
          `Resi ${trackingNorm} sudah pernah dicetak pada ${new Date(existing.printed_at).toLocaleString('id-ID')}`,
          409
        );
      }
      // Existing record without print (e.g. packed directly) — set print info retroactively
      const now = new Date();
      const wita = getWita();
      await db.collection('om_shipments').updateOne(
        { id: existing.id },
      ...
      return jsonRes({
        ...
        message: `Resi ${trackingNorm} dicatat sebagai tercetak (sudah ada di fase ${existing.status}).`,
    ...
    const shipment = {
      ...
      // Print phase only — pack fields not yet filled
      expedition_id: null,
    ...
    return jsonRes({
      ...
      message: `Resi ${trackingNorm} berhasil dicatat sebagai tercetak.`,
    });
  }
  ...
  if (subPath === 'scan/pack' && method === 'POST') {
    ...
    const now = new Date();
    let shipment;
    if (upgradingFromPrint) {
      // Upgrade printed → packed
      const updates = {
      ...
      await db.collection('om_shipments').updateOne({ id: dup.id }, { $set: updates });
      shipment = { ...dup, ...updates };
    } else {
      shipment = {
        id: uuidv4(),
        tracking_number: trackingNorm,
        ...
        sku_count: Math.floor(skuNum),
        item_count: Math.floor(itemNum),
        status: 'packed', // printed → packed → delivered
        // Print phase never happened
        printed_by_id: null,
        printed_by_name: null,
        printed_at: null,
        printed_wita_date: null,
    ...
    return jsonRes({
      ...
      message: upgradingFromPrint
        ? `Resi ${trackingNorm} berhasil dipacking · ${exp.name} (sudah dicetak sebelumnya)`
        : `Resi ${trackingNorm} berhasil dipacking · ${exp.name}`,
      upgraded_from_print: upgradingFromPrint,
    });
  }
  ...
  if (subPath === 'scan/deliver' && method === 'POST') {
    ...
    if (doc.status === 'printed') {
      return err(`Resi ${trackingNorm} baru tercetak, belum dipacking. Harap dipacking terlebih dahulu.`, 409);
    }
