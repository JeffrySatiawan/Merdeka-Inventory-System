...
export async function maybeRunOMCleanup(db) {
  ...
  try {
    ...
    // Purge expired PDFs — uses its OWN retention setting `pdf_retention_days`
    // (default 7) so PDF Resi History can survive longer than the photo cap.
    // Fallback chain: pdf_retention_days → photo_retention_days → 7.
    try {
      const pdfTtl = Number(s.pdf_retention_days ?? s.photo_retention_days ?? 7);
      const pdfCutoff = new Date(Date.now() - pdfTtl * 86400000);
      const expiredPdfs = await db
        .collection('om_pdfs')
        .find({ uploaded_at: { $lt: pdfCutoff }, deleted: { $ne: true } })
        .toArray();
...
export async function handleOMRequest(req, subPath, method, { db, user }) {
  ...
  if (subPath === 'expeditions' && method === 'POST') {
    ...
    const { name, code = '', express_id = '', active = true, sort_order = 99, service_type } = body || {};
    ...
    // service_type baru — kategori pengiriman INSTAN atau REGULER (default 'reguler').
    // Dipakai oleh Scan Cetak Resi (staff pilih INSTAN/REGULER) dan Serah Terima
    // Kurir (staff pilih ekspedisi aktual yang cocok kategori-nya).
    const st = String(service_type || 'reguler').toLowerCase();
    const doc = {
      id: uuidv4(),
      name: String(name).trim(),
      code: String(code).trim().toUpperCase(),
      express_id: String(express_id).trim(),
      service_type: st === 'instan' ? 'instan' : 'reguler',
      active: !!active,
  ...
  if (expMatch && method === 'PUT') {
    ...
    if (body.service_type !== undefined) {
      const st = String(body.service_type || 'reguler').toLowerCase();
      update.service_type = st === 'instan' ? 'instan' : 'reguler';
    }
  ...
  if (subPath === 'settings' && method === 'PUT') {
    ...
    if (body.pdf_retention_days !== undefined) {
      const v = Math.max(1, Math.min(365, Number(body.pdf_retention_days) || 7));
      update.pdf_retention_days = v;
    }
  ...
  // Setelah patch INSTAN/REGULER: staff pilih KATEGORI di menu Scan Cetak Resi
  // (bukan ekspedisi). Body baru: { tracking_number, service_type: 'instan'|'reguler' }.
  // Ekspedisi aktual dipilih di Serah Terima Kurir (endpoint scan/deliver).
  // Backward compat: body lama {tracking_number, expedition_id} tetap didukung.
  if (subPath === 'scan/print' && method === 'POST') {
    const body = await req.json();
    const { tracking_number, expedition_id, service_type: stIn } = body || {};
    ...
    // Prefer service_type; fallback ke expedition_id (legacy caller).
    let serviceType = null;
    let legacyExp = null;
    if (stIn) {
      const st = String(stIn).toLowerCase();
      if (st !== 'instan' && st !== 'reguler') return err('service_type harus "instan" atau "reguler"');
      serviceType = st;
    } else if (expedition_id) {
      legacyExp = await db.collection('om_expeditions').findOne({ id: expedition_id });
      if (!legacyExp) return err('Ekspedisi tidak ditemukan', 404);
      serviceType = legacyExp.service_type === 'instan' ? 'instan' : 'reguler';
    } else {
      return err('Pilih INSTAN atau REGULER terlebih dahulu');
    }
    ...
    if (existing) {
      ...
      await db.collection('om_shipments').updateOne(
        { id: existing.id },
        {
          $set: {
            ...
            ...(legacyExp && !existing.expedition_id ? {
              expedition_id: legacyExp.id,
              expedition_name: legacyExp.name,
              expedition_code: legacyExp.code,
            } : {}),
          },
        }
      );
      ...
      return jsonRes({
        ...
        message: `Resi ${trackingNorm} · ${serviceType.toUpperCase()} · tercetak.`,
      });
    }
    ...
    const shipment = {
      id: uuidv4(),
      tracking_number: trackingNorm,
      // Kategori pengiriman disimpan sejak print. Ekspedisi aktual DIISI
      // saat Serah Terima Kurir (kecuali legacy caller yang kirim expedition_id).
      service_type: serviceType,
      expedition_id: legacyExp ? legacyExp.id : null,
      expedition_name: legacyExp ? legacyExp.name : null,
      expedition_code: legacyExp ? legacyExp.code : null,
      sku_count: 0,
      item_count: 0,
      status: 'printed',
      printed_by_id: user.id,
  ...
  if (subPath === 'scan/pack' && method === 'POST') {
    ...
    if (opMode === 'serah_terima') {
      ...
      // URUTAN WAJIB: Dokumentasi Packing hanya boleh setelah Serah Terima Barang.
      // Reject dini bila resi ini belum melewati tahap Serah Terima. Cek pakai
      // field existing `serah_terima_at` (di-set di cabang serah_terima).
      if (!doc.serah_terima_at) {
        return NextResponse.json(
          {
            error: 'Resi belum melewati proses Serah Terima Barang.',
            missing_stage: 'serah_terima',
          },
          { status: 409 }
        );
      }
  ...
  // Setelah patch: staff pilih ekspedisi AKTUAL di menu ini (bukan lagi saat
  // Cetak Resi). Body: { tracking_number, expedition_id }. Validasi:
  // expedition.service_type harus cocok dengan resi.service_type.
  if (subPath === 'scan/deliver' && method === 'POST') {
    ...
    // Resolve ekspedisi. Kalau resi belum punya ekspedisi (alur baru
    // INSTAN/REGULER), expedition_id WAJIB. Kalau resi sudah punya
    // (legacy path), expedition_id opsional — dipakai kalau owner mau ganti.
    let expUpdates = {};
    if (expedition_id) {
      const exp = await db.collection('om_expeditions').findOne({ id: expedition_id });
      if (!exp) return err('Ekspedisi tidak ditemukan', 404);
      // Validasi kategori: cegah salah pilih ekspedisi INSTAN untuk resi REGULER
      // (dan sebaliknya). Kalau resi.service_type null (data lama), lewati.
      if (doc.service_type && exp.service_type && doc.service_type !== exp.service_type) {
        return err(
          `Ekspedisi "${exp.name}" adalah kategori ${String(exp.service_type).toUpperCase()}, ` +
          `sedangkan resi ini kategori ${String(doc.service_type).toUpperCase()}.`
        );
      }
      expUpdates = {
    ...
    } else if (!doc.expedition_id) {
      // Resi butuh pilihan ekspedisi. Kembalikan info resi + kategori supaya
      // frontend bisa menampilkan picker ekspedisi yang cocok. Bukan error —
      // ini step normal pada workflow INSTAN/REGULER.
      return jsonRes({
        needs_expedition: true,
        shipment: {
          id: doc.id,
  ...
  if (subPath === 'dashboard' && method === 'GET') {
    ...
    // Breakdown by service_type (hari ini) — untuk kontrol jumlah INSTAN /
    // REGULER antara Scan Cetak & Scan Serah Terima Kurir.
    const bySvcAgg = await db
      .collection('om_shipments')
      .aggregate([
        { $match: {
          $or: [
        ...
        { $group: {
          _id: { $ifNull: ['$service_type', 'reguler'] },
          printed: { $sum: { $cond: [{ $eq: ['$printed_wita_date', today] }, 1, 0] } },
          delivered: { $sum: { $cond: [{ $eq: ['$delivered_wita_date', today] }, 1, 0] } },
        } },
      ])
      .toArray();
    const svcMap = { instan: { printed: 0, delivered: 0 }, reguler: { printed: 0, delivered: 0 } };
    for (const r of bySvcAgg) {
      const k = r._id === 'instan' ? 'instan' : 'reguler';
      svcMap[k] = { printed: r.printed || 0, delivered: r.delivered || 0 };
    }
    ...
    return jsonRes({
      ...
      by_service_type: [
        { service_type: 'instan', ...svcMap.instan, diff: svcMap.instan.printed - svcMap.instan.delivered },
        { service_type: 'reguler', ...svcMap.reguler, diff: svcMap.reguler.printed - svcMap.reguler.delivered },
      ],
...
async function handlePDFRequest(req, subPath, method, { db, user }) {
  ...
  if (pdfPrintMatch && method === 'POST') {
    ...
    // Print protection: karyawan (non-owner) hanya boleh cetak SATU KALI per PDF.
    // Owner: unlimited (perilaku lama dipertahankan).
    // Additive check — tidak mengubah database schema. Menggunakan field
    // `printed_at` yang sudah ada.
    const isOwnerRole = user?.role === 'owner';
    if (!isOwnerRole && doc.printed_at) {
      return NextResponse.json(
        {
          error: 'PDF sudah pernah dicetak. Karyawan hanya boleh mencetak satu kali per PDF.',
          already_printed: {
            printed_at: doc.printed_at,
            printed_by_name: doc.printed_by_name,
          },
        },
        { status: 403 }
      );
