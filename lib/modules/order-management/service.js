...
export async function handleOMRequest(req, subPath, method, { db, user }) {
  ...
  if (subPath === 'scan/pack' && method === 'POST') {
    ...
    const now = new Date();
    const updates = {
      // Persist null when the caller didn't supply counts (Menu 2 photo-only
      // flow). Menu 1 continues sending valid integers, stored as before.
      sku_count: hasSku ? Math.floor(skuNum) : null,
      item_count: hasItem ? Math.floor(itemNum) : null,
      status: 'packed',
