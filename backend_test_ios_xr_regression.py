#!/usr/bin/env python3
"""
QUICK REGRESSION TEST — Follow-up iOS Compression Patch (iPhone XR / 17 Pro Max)

CONTEXT:
Minimal follow-up patch on /app/components/modules/order-management/api.js — only 3 numeric 
constants changed in the existing safety loop (budget 8→25, quality floor 0.3→0.2). 
No structure changes. Backend is UNTOUCHED.
...
TEST SET:
1. WebP photo upload path still works
2. JPEG photo upload path still works
3. PNG photo upload path still works (legacy)
4. >500KB payload STILL rejected (existing cap)
5. Zero regression in other endpoints
6. Cleanup
"""
...
TEST_TRACKING_WEBP = "IOS-XR-WEBP-001"
TEST_TRACKING_JPEG = "IOS-XR-JPEG-001"
TEST_TRACKING_PNG = "IOS-XR-PNG-001"
TEST_TRACKING_OVERSIZED = "IOS-XR-OVERSIZED-001"
...
def test_endpoint_regression(token):
    ...
    # Test 3: GET /api/om/packing-productivity
    try:
        resp = requests.get(
    ...
    # Test 5: POST /api/om/pdfs/{id}/mark-printed (from previous PDF print protection patch)
...
def main():
    print(f"\n{'#'*80}")
    print(f"# QUICK REGRESSION TEST — iOS Compression Patch (iPhone XR / 17 Pro Max)")
    ...
    print_test_header(1, "WEBP PHOTO UPLOAD PATH STILL WORKS")
    ...
    print_test_header(2, "JPEG PHOTO UPLOAD PATH STILL WORKS")
    ...
    print_test_header(3, "PNG PHOTO UPLOAD PATH STILL WORKS (LEGACY)")
    ...
    print_test_header(4, ">500KB PAYLOAD STILL REJECTED (EXISTING CAP)")
    ...
    if all_tests_passed:
        print("✅ ALL 6 TESTS PASSED (100%)")
        print("\nCRITICAL SUCCESS CRITERIA (ALL MET):")
        print("✅ Photo upload (WebP + JPEG + PNG) → 200")
        print("✅ >500KB → 400 (backend cap intact)")
        print("✅ No regression in ANY other endpoint")
        print("\nCONCLUSION:")
        print("The iOS compression patch (budget 8→25, quality floor 0.3→0.2) is FULLY WORKING.")
